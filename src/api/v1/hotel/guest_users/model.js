...
const guestUserDetails = new mongoose.Schema({
    guest_id: {
        type: String,
        default: uuidv4,
    },
    ...
    guest_name: { type: fullName, required: true },
    username: { type: String, required: false },
    guest_password: { type: String, required: true },
    guest_role: { type: [String], default: "Guest User" },
    civil_status: { type: String, required: false },
    current_full_address: { type: fullAddress },
    guest_nationality: { type: String, required: false },
    guest_religion: { type: String, required: false },
    guest_education_attainment: { type: [educationStructureSchema], required: true },
...
guestUserDetails.pre('save', async function (next) {
    // Check for duplicate email
    const existingEmail = await GuestUserAccount.findOne({ email_address: this.email_address });
    if (existingEmail) {
...
const GuestUserAccount = mongoose.model('hotel_guest_users', guestUserDetails);

module.exports = GuestUserAccount;